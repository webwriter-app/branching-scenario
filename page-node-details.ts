...
export class PageNodeDetails extends LitElementWw {
  ...
  render() {
    return html`<div id="sheetControls">
          ...
          ${Object.keys(this.nodesInEditor).map(
            (key) =>
              html`<sl-option value=${this.nodesInEditor[key].id}
                >${this.nodesInEditor[key].data.name}</sl-option
              >`
          )}
        </sl-select>
      ...
      <p>Selected Worksheet: ${this.selectedNode.data.name}</p>
      ...
      <sl-textarea
        ...
        size="large"
        .value="${this.selectedNode.data.html}"
  ...
  private _connectSelectedNodes() {
    ...
    //insert a button to the selected Nodes data.html
    const currentHtml = this.textAreaHTML.value;
    ...
    if (closingDivIndex !== -1) {
      ...
      this.textAreaHTML.value = newHtml;
      ...
      //   this.gamebook.saveChangesToPageContent(this.selectedNode.id, newHtml);
